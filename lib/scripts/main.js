import { system, EquipmentSlot, ItemStack, EntityComponentTypes, } from "@minecraft/server";
import { MessageFormData } from "@minecraft/server-ui";
const POTION_EFFECT_MAP = {
    // --- Normal potions ---
    "minecraft:swiftness": {
...
function isPotionItem(typeId) {
    return (typeId === "minecraft:potion" ||
    ...
    for (const target of nearbyPlayers) {
        for (const effect of entry.effects) {
            try {
                ...
                // Instant effects (instant_health, instant_damage) need duration=1
                try {
...
// ============================================================
// Section 3: Custom Component Registration
// ============================================================
system.beforeEvents.startup.subscribe((event) => {
    event.blockComponentRegistry.registerCustomComponent("potionarea:totem_alquimia", {
        onPlayerInteract(e) {
...
// ============================================================
// Section 4: Interaction Handler
// ============================================================
function handleTotemInteraction(player, blockLocation, dimension) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!item || !isPotionItem(item.typeId)) {
            player.sendMessage("§cSegure uma pocao na mao para usar o Totem de Alquimia!");
            return;
        }
        const potionComp = item.getComponent("minecraft:potion");
        if (!potionComp) {
            player.sendMessage("§cEsta pocao nao possui dados de efeito.");
            return;
        }
        const potionEffectType = potionComp.potionEffectType;
        const potionEffectId = potionEffectType.id;
        const durationTicks = (_a = potionEffectType.durationTicks) !== null && _a !== void 0 ? _a : 600;
        const entry = POTION_EFFECT_MAP[potionEffectId];
        if (!entry) {
            player.sendMessage(`§cPocao desconhecida: ${potionEffectId}`);
            return;
        }
        if (entry.effects.length === 0) {
            player.sendMessage(`§ePocao de ${entry.displayName} nao possui efeito para aplicar.`);
            return;
        }
        const form = new MessageFormData()
            .title("Totem de Alquimia")
            .body(`§eDeseja aplicar §b${entry.displayName}§e a todos os jogadores num raio de 10 blocos?\n\n` +
            `§7Duracao: ${(durationTicks / 20).toFixed(1)}s`)
            .button1("Confirmar")
            .button2("Cancelar");
        const response = yield form.show(player);
        if (response.canceled || response.selection !== 0) {
            player.sendMessage("§7Acao cancelada.");
            return;
        }
        // Re-verify the potion is still in hand
        const currentItem = equippable.getEquipment(EquipmentSlot.Mainhand);
        if (!currentItem || !isPotionItem(currentItem.typeId)) {
            player.sendMessage("§cVoce nao esta mais segurando uma pocao!");
            return;
        }
        ...
        if (!currentPotionComp || currentPotionComp.potionEffectType.id !== potionEffectId) {
            player.sendMessage("§cA pocao na sua mao mudou!");
            return;
        }
        // Consume the potion
        consumePotion(player, currentItem);
        // Apply effects to nearby players using the BLOCK location as center
        const affectedCount = applyPotionEffectsToNearby(blockLocation, dimension, potionEffectId, durationTicks);
        player.sendMessage(`§aEfeito §b${entry.displayName}§a aplicado a §e${affectedCount}§a jogador(es) proximo(s)!`);
    });
}
