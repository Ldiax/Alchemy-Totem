...
// ============================================================
// Section 1: Potion Effect Map
// ============================================================

interface PotionEffectEntry {
  effects: { effectId: string; amplifier: number }[];
  displayName: string;
}

const POTION_EFFECT_MAP: Record<string, PotionEffectEntry> = {
  // --- Normal potions ---
  "minecraft:swiftness": {
...
function isPotionItem(typeId: string): boolean {
  return (
...
function consumePotion(player: Player, item: ItemStack): void {
...
function applyPotionEffectsToNearby(
...
async function handleTotemInteraction(
  player: Player,
  blockLocation: Vector3,
  dimension: Dimension
): Promise<void> {
  ...
  const form = new MessageFormData()
    ...
    .body(
      `§eDeseja aplicar §b${entry.displayName}§e a todos os jogadores num raio de 10 blocos?\n\n` +
  ...
  player.sendMessage(
    `§aEfeito §b${entry.displayName}§a aplicado a §e${affectedCount}§a jogador(es) proximo(s)!`
  );
}
